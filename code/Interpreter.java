...
public class Interpreter extends ASTBaseVisitor<Void> {
	...
	protected Void visitIf(AST node) {
		...
		if (test == 1) { // THEN
			...
			int childEven = node.getChildCount() % 2 == 1 ? node.getChildCount() - 1 : node.getChildCount();
			for (int i = 2; i < childEven; i++) {
				visit(node.getChild(i));
				test = stack.popi();				
				if(test == 1){
		...
		if (test == 0 && node.getChildCount() % 2 == 1) { // ELSE, se houver
			visit(node.getChild(node.getChildCount() - 1));
		}
		return null; // Java exige um valor de retorno mesmo para Void... :/
	}
	...
	@Override
	protected Void visitLtEq(AST node){
		AST lexpr = node.getChild(0);
		AST rexpr = node.getChild(1);
		visit(lexpr);
		visit(rexpr);
	...
	@Override
	protected Void visitGtEq(AST node){
		AST lexpr = node.getChild(0);
		AST rexpr = node.getChild(1);
		visit(lexpr);
		visit(rexpr);
	...
	protected Void visitPrint(AST node) {
		...
		switch(expr.type) {
	        ...
	        case STR_TYPE:  writeStr(); System.out.println();   break;
			case NO_TYPE:
		    default:
	            System.err.printf("Invalid type: %s!\n", expr.type.toString());
	            System.exit(1);
